...
::substrait::Type DuckDBToSubstrait::DuckToSubstraitType(const LogicalType &type, BaseStatistics *column_statistics_p,
                                                         bool not_null) {
	...
	case LogicalTypeId::UNION: {
		auto union_type = new substrait::Type_UserDefined;
		union_type->set_nullability(type_nullability);
		s_type.set_allocated_user_defined(union_type);
		return s_type;
	}
...
//	auto &table = table_scan_bind_data.table;
	sget->mutable_named_table()->add_names("example_table");
...
//	auto not_null_constraint = GetNotNullConstraintCol(table);
	for (idx_t i = 0; i < dget.names.size(); i++) {
		...
		*new_type = DuckToSubstraitType(cur_type, nullptr, not_null);
	}
...
substrait::Rel *DuckDBToSubstrait::TransformGet(LogicalOperator &dop) {
	...
	printf("Loooking for filters\n");
	if (!dget.table_filters.filters.empty())
	{
		printf("Doing filters\n");
		// Pushdown filter
		auto filter =
	...
	if (!dget.projection_ids.empty()) {
		...
		auto &column_ids = dget.column_ids;
...
substrait::RelRoot *DuckDBToSubstrait::TransformRootOp(LogicalOperator &dop) {
	...
	auto &dproj = (LogicalGet &)*current_op;
	if (!weird_scenario) {
